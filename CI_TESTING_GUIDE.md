...
| backend-tests | `.github/workflows/backend-tests.yml` | PR + push | MariaDB 11 خدمة → `prisma migrate deploy` على `mjm_store_test` → jest unit → jest e2e |
| playwright | `.github/workflows/playwright.yml` | PR + push | MariaDB + migrate + `seed_all.ts` → Chromium: كاملة مع ADMIN_* أو عامة فقط بدونهما |
...
|---|---|---|
| `DB_ROOT_PASSWORD` | كلمة مرور root لخدمة MariaDB (أو `DATABASE_URL_TEST` كاملًا بدلًا منها) | backend-tests, playwright, accessibility, lighthouse, security, k6 |
...
---
**English summary:** 7 workflows (quality, backend-tests, playwright, accessibility, lighthouse-manual, zap-baseline, k6-manual-only). Node 22, npm ci, MariaDB 11 service with isolated `mjm_store_test`, secrets by name only, Chromium-first, ZAP official Docker baseline on localhost, k6 dispatch-only with 429-tolerant/5xx-strict evaluation, artifacts per area, no app-logic changes.
